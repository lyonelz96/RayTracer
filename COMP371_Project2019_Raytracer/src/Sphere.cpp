...
bool Sphere::doesRayIntersect(Ray& ray, float &t)
{
	...
	*/
	...
	if (discriminant < 0.0) { // No roots
		...
		//Also checking for backface culling in all cases
		if (t0 > 0.0 && t1 > 0.0) {
			t = std::fminf(t0, t1); // Get the closest intersection point
			if (glm::dot(ray.getDirection(), (ray.calcPointAlongRay(t) - this->getPosition())) > 0) {
				return false;
			}
			return true;
		}
...
glm::vec3 Sphere::calcColor(Ray& ray, Light& light, Plane& plane, std::vector<Sphere*>& spheres, Mesh& mesh, float& t)
{
	...
	Ray shadowRay;
	shadowRay.setOrigin(ray.calcPointAlongRay(t) + ((ray.calcPointAlongRay(t) - this->getPosition()) * (float)1e-4)); //Origin moved up with bias, to avoid Shadow Acne
